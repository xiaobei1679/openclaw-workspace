...
1. **对"生成式 codegen"缓存要慎用。** `callLLM` 服务的是**代码生成**任务，缓存键 = `哈希(git 文件树 + AGENTS.md + 任务标题/正文 + model)`。这意味着：**同一仓库状态下重跑同一任务会命中缓存、原样返回上次生成结果，而不再调用 LLM**。若上次生成有 bug（parse 失败 / `node --check` 不过 / LLM 拒答），由于失败不改变 `git ls-files` 跟踪树，**立即重跑会拿到一模一样的失败输出，无法自愈**。缓解手段：默认 `LLM_CACHE_TTL=6h`（超时后可重新生成）；需要"每次都要求全新生成"时设 `LLM_CACHE=0`。**判断原则**：prompt 缓存最适合**幂等/确定性**子调用（路由、分类、摘要），对**创造性生成**应短 TTL 或关闭。
2. **熔断器是内存态单例，在当前"一次进程只调一次 LLM"的运行模型下几乎不生效。** `respond.mjs` 是一次性进程（一个 issue/本地任务 → 一次 `callLLM` → 退出），熔断器 `failureThreshold=3` 需要**同一进程内多次调用**才可能跳闸；单次调用后进程退出、内存态丢失，跨进程**不持久**。因此它目前对"每小时反复起进程打同一故障 provider"**不能**提供跨运行的雷鸣羊群防护——那需要把熔断状态持久化到磁盘（同缓存），或引入长驻多步 agent 循环。**现状定位**：熔断器是为**未来的多步 agent 循环**预留的正确机制，今天在单次进程里基本是"备而未用"，不构成危害但别高估其当下价值。
